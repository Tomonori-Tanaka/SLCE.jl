...
- `SALCKey` — the canonical structural identity of a basis column. Column order is
  ...
  construction order.
- Hot-path harmonics come in a validated `Zlm` and an unchecked, buffered
  `Zlm_unsafe` (likewise `grad_Zlm` / `grad_Zlm_unsafe`). The docstrings spell
  these `Zₗₘ` per §1.7 — the identifiers themselves are ASCII.
