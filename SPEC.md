...
- **Estimator** (`fitting/estimators.jl`, in-core): `GroupAdaptiveRidge(column_groups,
  group_weights; lambda, epsilon, max_iter, tol, metric, metric_provenance)` — the
  group extension of
  `AdaptiveRidge`, approximating the weighted group-L0 `λ·Σ_g v_g·1{β_g≠0}` by iterating
  `Dⱼ = mⱼ·wⱼ`, `wⱼ = v_g/(Σ_{k∈g} m_k·β_k² + p_g·ε)` (all columns of a group share one
  weight; a surviving
  group's converged penalty is exactly `λ·v_g`, with or without a metric). Exact
  degeneration to `AdaptiveRidge`
  ...
  weight map `_group_adaptive_weights!` is the single definition shared with the GCV
  diagnostics, and what it returns is the penalty **diagonal** `Dⱼ`, metric included.

### The penalty metric (`fitting/metric.jl`)
- **What** (exported): `penalty_metric(basis::SLCEBasis; torque_weight, nconfig = 2048,
  seed = 1)` and `penalty_metric(mb::MomentBasis; free_intercepts = true, nconfig, seed)`
  — the per-column scale `m` carried by `Ridge` / `AdaptiveRidge` / `GroupAdaptiveRidge`
  (field `metric`, `nothing` = uniform). `λ·Σβⱼ²` is not invariant under rescaling a
  design column and SALC column norms are basis conventions (orbit size × the member
  fold's ordering multiplicity), so the unweighted penalty carries an accidental prior
  toward large orbits and high body order. The energy metric is
  `mⱼ(w) = (1−w)·Var[Φⱼ] + w·(1/3n_at)·E[Σ_a ‖(∂Φⱼ/∂e_a)×e_a‖²]` — the reference norm of
  the column **as the assembled design presents it** — and the pointed metric is
  `mⱼ = E[Φⱼ²]` over the pointed rows (no centering; the moment design carries its
  intercepts as columns).
- **Where it enters**: the DENOMINATOR of every weight map, never outside it. Outside,
  the adaptive estimators would stop being scale invariant and the group-L0 fixed point
  would read `λ·v_g·⟨m⟩_g` instead of `λ·v_g`. Cold starts are built from the metric for
  the same reason; the IRLS stopping rule is measured in metric coordinates `√mⱼ·βⱼ` over
  the penalized columns only. Under an ASR / freeze reparameterization the penalty
  compresses to `Z'·Diagonal(D)·Z`, so `metric` stays indexed by the **basis** columns
  exactly as `column_groups` is; with no metric the uniform penalty stays `λ·I` in γ
  space (Z orthonormal), which keeps every unweighted fit bit-identical.
- **`mⱼ = 0` ⇒ unpenalized.** Reserved for a STRUCTURAL exemption — the pointed μ₀
  intercepts and the identically-vanishing columns — and never inferred from a sample
  (`_refuse_zero_metric`). The unpenalized block must have full column rank or the solve
  is refused by name (`_check_free_block`, an eigenvalue cut at `κ ≲ 6.7e7` on its Gram);
  `_effective_dof_free` gives the split-block dof `p_F + Σ sᵢ/(sᵢ+λ)`.
- **Scope: pure spin.** The reference ensemble is uniform random spin directions, so a
  displacement-decorated basis is refused by name and so is a fit carrying `force_weight
  > 0` under a metric — the reference distribution of `u` is a separate design decision.
  The reference ensemble is generated by an in-package SplitMix64 (`_reference_configs`),
  not by `Random`, so recorded penalized fits do not move between Julia versions.
- **Provenance**: `MetricProvenance(channel, torque_weight, nconfig, seed, fingerprint)`
  is stamped by the basis-aware constructors and checked at `fit` / `refit` /
  `select_fit` / `cross_validate` / `fit(MomentFit, …)`; a hand-built metric carries none
  and passes. `SLCE.with_lambda(est, λ)` re-lambdas an estimator with the metric and
  provenance intact — the λ-sweep idiom, since a hand rebuild drops both silently.
- **Tests**: analytic closed forms (`test/unit/test_selection.jl`: unpenalized ≡ OLS on
  an orthogonal design, scale invariance with an inhomogeneous rescaling + the
  uniform-metric control, the group-L0 fixed point, the `2√3(e₁·e₂)` reference norms
  `Var = 4` / `m(w=1) = 8/3`, the dense-hat dof reference, the Z'DZ closed form);
  pointed μ₀ exemption and the λ→∞ intercept-only limit in `test/unit/test_momentfit.jl`;
  threaded ≡ serial in `test/unit/test_threading.jl`.
